...
//**************************************************************
// Grabar FICHA DE PREINVERSION PROPIR
// 
//**************************************************************
...
// CONFIGURAR FECHAS  
  ...
  if ($xpreinversion==0) {
	 ...
	 $xpreinversion=substr($xpreinversion, strlen($xpreinversion)-4,4)+0;
  }  
  ...
  $Biniciativa->responsable_ejecucion=$_REQUEST["e_responsable_ejecucion"];
  $Biniciativa->c_programa_propir=$_REQUEST["e_programa"];
  ...
  $Biniciativa->c_estado_preinversion=$_REQUEST["nestadopropir"];
  ...
  $Biniciativa->usuario=$_REQUEST["id_usuario"];
  $error=$Biniciativa->actualizar_iniciativa();

  $contenedor_array_propir     = $_POST['contenedor_array'];
  ...
  if(!empty(contenedor_array_propir))
  {
    $Biniciativa->ingresa_georeferencia_propir($contenedor_array_propir);  
  }


  /* Fin modificacion georeferenciacion */

//  $contenedor_array_propir     = array_recibe($contenedor_array_propir);
//  $Biniciativa->ingresa_georeferencia_propir($contenedor_array_propir);

  if ($error!='') {
     echo $error;
  } else { 
//     $query = " REPLACE INTO RDR_PREINVERSION_PROPIR VALUES('";
...
// GRABAR RELACION INSTRUMENTOS
     $Binstrumento=new Relacion_Instrumentos_Iniciativa_Propir($BaseDatos->conector);
...
// GRABAR BENEFICIARIO
     $Bbeneficiario=new Beneficiario_Iniciativa_Propir($BaseDatos->conector);
   ...
   echo "parent.configurar_ficha();";
    echo "parent.document.form1.action=\"pagina1_1_3.php\";";   
 ...
 */
function array_recibe($contenedor_array_propir) { 
     $tmp = stripslashes($contenedor_array_propir); 
     $tmp = urldecode($tmp); 
     $tmp = unserialize($tmp); 
